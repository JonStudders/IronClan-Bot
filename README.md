...
Only the leaderboard table is read. A row counts as a team when it has a team
name **and** carries the points column — which is what keeps the other tables
further down the same tab (the WOM link, "Top EHB gainer per team") off the
board, even though some of their rows also carry a team name.
...
One embed, edited in place. Each team shows a medal or rank number, its
movement since the previous update, points, a completion bar and the gap to
the team above.
...
The bot keeps exactly one leaderboard message and keeps it at the bottom of the
channel. Any extra board it finds - left over from a crash mid-repost, or from
a retired layout - is deleted so only the newest survives.
...
## Project layout
    ...
    src/embed.js        Renders the Discord embed
    src/poster.js       Owns the leaderboard message (post once, then edit)
